...
curl -1sLf 'https://dl.cloudsmith.io/public/nxadm-pkgs/rakudo-pkg/config.deb.txt?distro=ubuntu&codename=xenial&component=main' > /etc/apt/sources.list.d/nxadm-pkgs-rakudo-pkg.list
sudo chmod 644 ${keyring_location}
sudo chmod 644 /etc/apt/sources.list.d/nxadm-pkgs-rakudo-pkg.list
apt-get update
